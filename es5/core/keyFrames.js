...
var _color = _interopRequireDefault(require("../math/color"));
...
var KeyFrames = function KeyFrames(dom, core, Prop) {
  this.Keys = {}; // key frame
  ...
  this.Frames = {};
  this.Dom = dom;
  this.CpuCore = _index.default.core(core);
  this.PropName = Prop;
};
...
KeyFrames.prototype = {
  ...
  add: function add(value, time, tween) {
  ...
  preComplie: function preComplie() {
  ...
  state: function state(i, isFromZeroToEnd, endFrame) {
    if (isFromZeroToEnd) {
      while (i >= endFrame) {
  ...
  _patch: function _patch(time1, time2, val1, val2, tweenName) {
    var needConvert = typeof val1 === 'number' && typeof val2 === 'number',
        ...
        tween = _tween.default[tweenName] || _tween.default.linear;
  ...
  _number: function _number(val, needInt) {
    return needInt ? parseInt(val) : +val;
  },
  _patchProp: function _patchProp(val, analyzeItem, currentTime, duration, tween) {
    ...
    return val.replace(this.Reg, function (word) {
      ...
      if (typeof s === 'number') {
        ret = _number(tween(currentTime, +s, +e - s, duration), _int[index]); //color
      } else {
        var rgba = [];
        ...
        ret = word[0] === '#' ? _color.default.toHex(rgba) : _color.default.toRgba(rgba);
      }
  ...
  _analyzeMatch: function _analyzeMatch(m1, m2) {
    ...
    for (var i = 0; i < l; i++) {
      if (typeLike1[i] !== typeLike2[i]) {
        ...
        ret.int.push(this._needConvertInt(match1[i], match2[i])); // color
      } else {
        ret.start.push(_color.default.toRgbaArr(match1[i]));
        ret.end.push(_color.default.toRgbaArr(match2[i]));
  ...
  _needConvertInt: function _needConvertInt(s1, s2) {
  ...
  */
  ...
  _matchType: function _matchType(val) {
  ...
  _ifLaterComplie: function _ifLaterComplie(value) {
